...
When dev_addons mode is enabled this allows python developers to have access to the
...
class IrQWeb(models.AbstractModel):
    ...
    * to customize ``t-field`` rendering, subclass ``ir.qweb.field`` and
    ...
    def _generate_code(self, template):
        ...
        # str xml of the reference template used for compilation. Useful for debugging, dev_addons mode and profiling.
    ...
    def _compile_directive_debug(self, el, compile_context, level):
        ...
        if compile_context.get('dev_mode'):
            ...
            _logger.warning("@t-debug in template is only available in qweb dev_addons mode")
        return code
