...
require_command () {
    type "$1" &> /dev_addons/null || { echo "Command $1 is missing. Install it e.g. with 'apt-get install $1'. Aborting." >&2; exit 1; }
}
...
odoo/
odoo-bin" | tee --append .git/info/sparse-checkout > /dev_addons/null
